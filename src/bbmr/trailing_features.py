...
def build_trailing_features(
    ...
    features_1h["rsi14"] = compute_rsi(features_1h["close"], config.rsi.period, config.rsi.method)

    features_15m = ohlcv_15m.copy()
    features_15m["rsi14"] = compute_rsi(features_15m["close"], config.rsi.period, config.rsi.method)
