...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--timeframe", default="5m", choices=("1h", "15m", "5m"))
    ...
    try:
        ...
        for candle_open_at, completed_at, close, rsi in rows:
            print(
                f"candle_open_at_utc={candle_open_at.isoformat()} "
                f"candle_open_at_beijing={candle_open_at.tz_convert('Asia/Shanghai').isoformat()} "
                f"completed_at_utc={completed_at.isoformat()} "
                f"completed_at_beijing={completed_at.tz_convert('Asia/Shanghai').isoformat()} "
                f"close={close:.6f} rsi={rsi:.6f}"
            )
        return 0
...
def completed_rsi_rows(
    client, strategy_config, symbol: str, timeframe: str, count: int
    ...
    result = []
    for candle_open_at, completed_time, row, value in zip(
        _utc_index(frame.index)[usable], completed_at[usable], frame.loc[usable].itertuples(), rsi.loc[usable], strict=True
    ):
        if pd.notna(value):
            result.append((candle_open_at, completed_time, float(row.close), float(value)))
    return result[-count:]
