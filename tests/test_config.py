...
def test_loads_strategy_config():
    ...
    assert config.band_walking.enabled is False
    assert config.volume.entry_filter_enabled is False
    assert config.range_filter.entry_filter_enabled is False
    assert config.strategy.allow_live_trading is False
    assert config.safety.allow_live_trading is False


def test_loads_trailing_stop_initial_stop_pct():
...
def test_entry_filter_switches_default_true_for_legacy_config():
    data = _config_data()
    del data["volume"]["entry_filter_enabled"]
    del data["range_filter"]
    config = StrategyConfig.model_validate(data)
    assert config.volume.entry_filter_enabled is True
    assert config.range_filter.entry_filter_enabled is True


def test_band_walking_enabled_defaults_true_for_legacy_config():
    data = _config_data()
    del data["band_walking"]["enabled"]
    config = StrategyConfig.model_validate(data)
    assert config.band_walking.enabled is True
